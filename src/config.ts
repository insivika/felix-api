...
import {
   BossEventPerson,
   BossEventsCreateRequest,
   CustomBossField,
} from "./services/boss.js";
...
const envFilePath = path.join(
   __dirname,
   "..",
   process.env["DOT_ENV_CONFIG"] || ".env"
);
dotenv.config({
   path: envFilePath,
});
...
const parseOtpMessageType = (type: string | undefined): OtpMessageType => {
   if (
      type === undefined ||
      !otpMessageTypes.includes(type as OtpMessageType)
   ) {
...
const proximitySearchConfig = {
   ...
   radius_m: parseInt(
      process.env["PROXIMITY_SEARCH_RADIUS_METERS"] || "150000"
   ),
};
const config: AppConfig = {
   nats: {
      ...
      worker: {
         consumer_stream:
            process.env["NATS_WORKER_CONSUMER_STREAM"] || "boss-people",
         consumer_name:
            process.env["NATS_WORKER_CONSUMER_NAME"] || "boss-people-worker",
      },
   },
   ...
   auth: {
      agents_signature_salt:
         process.env["AUTH_AGENTS_SIGNATURE_SALT"] || "repliers123",
      otp: {
         ttl_ms: parseInt(process.env["AUTH_OTP_TTL_MS"] || "600000"),
         // 10 min
         resend_ttl_ms: parseInt(
            process.env["AUTH_OTP_RESEND_TTL_MS"] || "60000"
         ),
         // 1 min
         uri:
            process.env["AUTH_OTP_URI"] ||
            "http://localhost:3000/auth/otp/processing",
         message:
            process.env["AUTH_OTP_MESSAGE"] ||
            "Please click on the link below to login",
         message_type: parseOtpMessageType(
            process.env["AUTH_OTP_MESSAGE_TYPE"]
         ),
         debug_expose_code:
            process.env["DEBUG_AUTH_OTP_EXPOSE_CODE"] === "true",
      },
      emailtoken: {
         enabled: process.env["AUTH_EMAILTOKEN_ENABLED"] === "true",
         expire: (process.env["AUTH_EMAILTOKEN_EXPIRE"] ||
            "30d") as StringValue,
   ...
   app: {
      ...
      cors: {
         validDomains: [
            "https://portal.repliers.com/",
            ...(
               process.env["APP_CORS_DOMAIN"] || "http://localhost:3000"
            ).split(","),
         ],
      },
   },
   ...
   cache: {
      ...
      neighborhoodsranking: {
         ttl_ms: parseInt(
            process.env["CACHE_NEIGHBORHOODSRANKING_TTL"] || "86400000"
         ), // 24 * 60 * 60 * 1000 = 24 hours
      },
      autosuggest_locations: {
         ttl_ms: parseInt(
            process.env["CACHE_AUTOSUGGEST_LOCATIONS_TTL"] || "86400000"
         ), // 24 * 60 * 60 * 1000 = 24 hours
      },
      listingscount: {
         ttl_ms: parseInt(
            process.env["CACHE_LISTINGS_COUNT_TTL"] || "86400000"
         ), // 24 * 60 * 60 * 1000 = 24 hours
      },
   },
   repliers: {
      ...
      clients: {
         ...
         unauthenticatedClientId: parseInt(
            process.env["REPLIERS_UNAUTHENTICATED_CLIENT_ID"] || "0"
         ),
         defaultAgentEmail: process.env["REPLIERS_AGENT_EMAIL"],
      },
      autosuggest: {
         max_results: parseInt(
            process.env["REPLIERS_AUTOSUGGEST_MAX_RESULTS"] || "10"
         ),
         max_results_default: parseInt(
            process.env["REPLIERS_AUTOSUGGEST_MAX_RESULTS_DEFAULT"] || "3"
         ),
         lat: proximitySearchConfig.lat,
         long: proximitySearchConfig.long,
         radius: proximitySearchConfig.radius_m,
         provider:
            process.env["REPLIERS_AUTOSUGGEST_PROVIDER"] === "googlemaps"
               ? "googlemaps"
               : "mapbox",
      },
      ...
      estimatesNotificationSettings: {
         sendEmailNow:
            typeof process.env["REPLIERS_ESTIMATES_SEND_EMAIL_NOW"] !==
            "undefined"
               ? process.env[
                    "REPLIERS_ESTIMATES_SEND_EMAIL_NOW"
                 ].toLocaleLowerCase() === "true"
               : undefined,
         sendEmailMonthly:
            typeof process.env["REPLIERS_ESTIMATES_SEND_EMAIL_MONTHLY"] !==
            "undefined"
               ? process.env[
                    "REPLIERS_ESTIMATES_SEND_EMAIL_MONTHLY"
                 ].toLocaleLowerCase() === "true"
               : undefined,
      },
   },
   ...
   boss: {
      ...
      base_url:
         process.env["BOSS_BASEURL"] || "https://api.followupboss.com/v1",
      ...
      webhook: {
         ...
         base_url:
            process.env["BOSS_WEBHOOK_BASEURL"] ||
      ...
      custom_AVM_field: process.env["BOSS_CUSTOM_AVM_FIELD"] as
         | CustomBossField
         | undefined,
   },
   ngrok: {
   ...
   eventsCollection: {
      ...
      defaultPersonFields: {
         ...
         tags: process.env["BOSS_DEFAULT_TAGS"]?.split(",") || [
            process.env["APP_ENVIRONMENT"] || "dev",
         ],
      },
      defaultBoardId:
         process.env["SETTINGS_EVENTS_COLLECTOR_DEFAULT_BOARD_ID"] || "12",
      urlHost: process.env["SETTINGS_EVENTS_COLLECTOR_URL_HOST"] || undefined,
      propertyUrl:
         process.env["BOSS_PROPERTY_URL"] ||
         "https://example.tld/listing/[MLS_NUMBER]?boardId=[BOARD_ID]",
      clientUrl:
         process.env["BOSS_CLIENT_URL"] ||
         "https://example.tld/agent/client/[CLIENT_ID]",
      estimateUrl:
         process.env["BOSS_ESTIMATE_URL"] ||
         "https://example.tld/estimate/[ESTIMATE_ID]",
      savedSearchUrl: process.env["BOSS_SAVED_SEARCH_URL"] || "",
      customFieldsStrategy: process.env["BOSS_CUSTOM_FIELDS_STRATEGY"],
      reportClientViewEstimate:
         process.env["BOSS_REPORT_CLIENT_VIEW_ESTIMATE"] === "true",
   },
   settings: {
      max_estimate_id: parseInt(
         process.env["SETTINGS_MAX_ESTIMATE_ID"] || "100000"
      ),
      scrubbing_ref_board_id: parseInt(
         process.env["DUPLICATES_REFERENCE_BOARD_RESOURCE_ID"] || "9997"
      ),
      scrubbing_board_ids: (process.env["SETTINGS_SCRUBBING_BOARDIDS"] || "12")
         .split(",")
         .map((v) => parseInt(v)),
      scrubbing_duplicates_enabled:
         process.env["SETTINGS_SCRUBBING_PROCESS_DUPLICATES_ENABLED"] !==
         "false",
      scrubbing_force_display_public_yes:
         process.env["SETTINGS_SCRUBBING_FORCE_DISPLAY_PUBLIC_YES"] === "true",
      defaults: {
         boardId: (process.env["APP_DEFAULTS_BOARDIDS"] || "110")
            .split(",")
            .map((v) => parseInt(v)),
         locations_boardId: parseInt(
            process.env["APP_LOCATIONS_BOARDID"] || "110"
         ),
      },
      locations: {
         drop_coordinates:
            process.env["SETTINGS_LOCATIONS_DROP_COORDINATES"] === "true",
         allow_all_areas:
            process.env["SETTINGS_LOCATIONS_ALLOW_ALL_AREAS"] === "true",
         allowed_areas: (process.env["SETTINGS_LOCATIONS_ALLOWED_AREAS"] || "")
            .split(",")
            .map((v) => v.toLowerCase()),
         boardId: parseInt(process.env["APP_LOCATIONS_BOARDID"] || "110"),
         active_count_limit: parseInt(
            process.env["SETTINGS_LOCATIONS_ACTIVE_COUNT_LIMIT"] || "5"
         ),
      },
      hide_unavailable_listings_statuses: process.env[
         "APP_HIDE_UNAVAILABLE_LISTINGS_STATUSES"
      ]
         ? process.env["APP_HIDE_UNAVAILABLE_LISTINGS_STATUSES"].split(",")
         : ["Ter"],
      hide_unavailable_listings_http_code: process.env[
         "APP_HIDE_UNAVAILABLE_LISTINGS_HTTP_CODE"
      ]
         ? parseInt(process.env["APP_HIDE_UNAVAILABLE_LISTINGS_HTTP_CODE"])
         : 410,
      validationVersion: process.env["SETTINGS_VALIDATION_VERSION"],
      extended_property_details:
         process.env["SETTINGS_EXTENDED_PROPERTY_DETAILS"] === "true",
   },
   xff: {
...
if (config.boss.webhook.enabled) {
   assert(
      config.boss.system !== "",
      "Please, specify some unique BOSS_SYSTEM to avoid issues with outher developers"
   );
...
if (process.env["APP_ENVIRONMENT"] !== "localhost") {
   assert(
      ...
      "No GOOGLE_CREDENTIALS env variable"
   );
}
if (
   process.env["DEBUG_AUTH_OTP_EXPOSE_CODE"] === "true" &&
   process.env["APP_ENVIRONMENT"]?.includes("prod")
) {
